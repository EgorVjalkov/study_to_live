...
from states import FillingVedomost, FillingSleeptime
...
async def start_dialog(m: Message, dm: DialogManager):
    ...
    await dm.start(FillingVedomost.date_menu,
...
async def start_dialog(m: Message, dm: DialogManager):
    ...
    await dm.start(FillingVedomost.date_menu,
                   mode=StartMode.RESET_STACK,
                   data={'filler': VedomostFiller(recipient, 'correct').__call__()},
                   )
