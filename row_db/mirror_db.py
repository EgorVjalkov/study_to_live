...
from unfilled_rows_db import UnfilledRowsDB
...
class Mirror:
    ...
    @property
    def need_init_temp_db(self):
        return self.months_db_paths is False

    def init_temp_dbs(self):
        for day in [week_before_day, today]:
            path = self.path_to.vedomost_by(day)
            # НУЖЕН ЛИ ТУТ ПКТЬ ДО АЗЫДАННЫХ
            mother_frame = self.load_('mf', by_path=path)
            temp_db = UnfilledRowsDB()
            # init_bd_by_db_cls
        pass
